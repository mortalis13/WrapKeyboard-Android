...
public class WrapKeyboard extends InputMethodService implements KeyboardView.OnKeyboardActionListener {
  ...
  private LatinKeyboard mRuKeyboard;
  private LatinKeyboard mTextEditKeyboard;
  private LatinKeyboard mLangSymbolsKeyboard;
  ...
  final static int MoveUp = 60002;
  final static int MoveLeft = 60004;
  final static int MoveDown = 60005;
  final static int MoveRight = 60006;
  
  final static int KeyHome = 60001;
  final static int KeyEnd = 60003;
  
  final static int KeySelect = 60011;
  
  final static int KeyAll = 60007;
  final static int KeyCopy = 60008;
  final static int KeyCut = 60009;
  final static int KeyPaste = 60010;
  ...
  public void onInitializeInterface() {
    ...
    mRuKeyboard = new LatinKeyboard(this, R.xml.kbd_ru);
    mTextEditKeyboard = new LatinKeyboard(this, R.xml.text_edit);
    mLangSymbolsKeyboard = new LatinKeyboard(this, R.xml.lang_symbols);

    mCurLangKeyboard = mQwertyKeyboard;
  }
...
//    return mCandidateView;
  ...
  private boolean translateKeyDown(int keyCode, KeyEvent event) {
    System.out.println("translateKeyDown-keyCode: " + keyCode);
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    ...
    case KeyEvent.FLAG_EDITOR_ACTION:
      System.out.println("FLAG_EDITOR_ACTION");
      break;
  ...
  public void onKey(int primaryCode, int[] keyCodes) {
    System.out.println("onKey-primaryCode: " + primaryCode);
...
//      System.out.println("KEYCODE_DELETE");
      handleBackspace();
      ...
      if (current == mSymbolsKeyboard || current == mSymbolsShiftedKeyboard || current == mTextEditKeyboard || current == mLangSymbolsKeyboard) {
        if (mCurLangKeyboard == mQwertyKeyboard)
          current = mQwertyKeyboard;
        else if (mCurLangKeyboard == mRuKeyboard)
          current = mRuKeyboard;
      } else {
    ...
    else if (primaryCode == LatinKeyboardView.SwitchLangKeyboardKey) {
      Keyboard current = mInputView.getKeyboard();
      ...
      mCurLangKeyboard = current;
      mInputView.setKeyboard(current);
    } 
    
    else if (primaryCode == LatinKeyboardView.OpenTextEditKeyboardKey) {
      Keyboard current = mTextEditKeyboard;
      mInputView.setKeyboard(current);
    }

    else if (primaryCode == LatinKeyboardView.LangSymbolsKeyboardKey) {
      Keyboard current = mLangSymbolsKeyboard;
      mInputView.setKeyboard(current);
    }

    else if (primaryCode == MoveLeft) {
//      InputConnection ic = getCurrentInputConnection();
//      int pos = ic.getTextBeforeCursor(Integer.MAX_VALUE, 0).length();
//      pos--;
//      ic.setSelection(pos, pos);
      
      sendDownUpKeyEvents(KeyEvent.KEYCODE_DPAD_LEFT);
    }
    else if (primaryCode == MoveUp) {
      sendDownUpKeyEvents(KeyEvent.KEYCODE_DPAD_UP);

    }
    else if (primaryCode == MoveRight) {
      sendDownUpKeyEvents(KeyEvent.KEYCODE_DPAD_RIGHT);
    }
    else if (primaryCode == MoveDown) {
      sendDownUpKeyEvents(KeyEvent.KEYCODE_DPAD_DOWN);
    }
    
    else if (primaryCode == KeyHome) {
      sendDownUpKeyEvents(KeyEvent.KEYCODE_MOVE_HOME);
    }
    else if (primaryCode == KeyEnd) {
      sendDownUpKeyEvents(KeyEvent.KEYCODE_MOVE_END);
    }
    
    else if (primaryCode == KeySelect) {
      int keyEventCode=KeyEvent.KEYCODE_SHIFT_LEFT; 
      long eventTime = SystemClock.uptimeMillis();
      InputConnection ic = getCurrentInputConnection();
  
      if(!LatinKeyboardView.selectionModeEnabled){ 
        ic.sendKeyEvent( new KeyEvent(eventTime, eventTime, KeyEvent.ACTION_DOWN, keyEventCode, 0, 0) );
        LatinKeyboardView.selectionModeEnabled=true;
      }
      else{
        ic.sendKeyEvent( new KeyEvent(eventTime, eventTime, KeyEvent.ACTION_UP, keyEventCode, 0, 0) );
        LatinKeyboardView.selectionModeEnabled=false;
      }
    }
    
    else if (primaryCode == KeyAll) {
      InputConnection ic = getCurrentInputConnection();
      ic.setSelection(0, 0);
      ic.performContextMenuAction(android.R.id.selectAll);
    }
    else if (primaryCode == KeyCopy) {
      InputConnection ic = getCurrentInputConnection();
...
//      clipboard.setPrimaryClip(clip);
    ...
    else if (primaryCode == KeyCut) {
      InputConnection ic = getCurrentInputConnection();
      ic.performContextMenuAction(android.R.id.cut);
    }
    else if (primaryCode == KeyPaste) {
      InputConnection ic = getCurrentInputConnection();
...
//      ic.commitText(pasteData, 1);
      ...
      if (mInputView.isShifted() && !mInputView.capsLock)
  ...
  private void handleShift() {
    ...
    if (mQwertyKeyboard == currentKeyboard || mRuKeyboard == currentKeyboard || mLangSymbolsKeyboard == currentKeyboard) {
      // Alphabet keyboard
      // checkToggleCapsLock(); 
      mInputView.setShifted(mCapsLock || !mInputView.isShifted());
      
      if(mInputView.capsLock)   
        mInputView.capsLock=false;
    } 
...
//    System.out.println("onPress-primaryCode: " + primaryCode);
